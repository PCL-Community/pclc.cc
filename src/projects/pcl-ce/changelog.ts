...
export function renderMD(markdown: string): string {
    ...
    if (matchedNames != null)
        for (let name of new Set(matchedNames)) {
            ...
            markdown = markdown.replaceAll(`**${name.substring(1)}**`, `[**${name}**](https://github.com/${name.substring(1)})`)
        }
    ...
    if (matchedLinks != null)
        for (let url of matchedLinks) {
            ...
            markdown = markdown.replaceAll(url, replaced.substring(replaced.indexOf('(') + 1, replaced.lastIndexOf(')')));
        }
    ...
    if (matchedUrls != null)
        for (let url of matchedUrls) {
            ...
            if (matchedIssues != null) 
                markdown = markdown.replaceAll(url, `[**\`Issue #${url.substring(url.lastIndexOf('/') + 1)}\`**](${url})`);
            if (matchedCommits != null) 
                markdown = markdown.replaceAll(url, `[**\`Commit ${url.substring(url.lastIndexOf('/') + 1, url.lastIndexOf('/') + 7)}\`**](${url})`);
            if (matchedCompare != null)  {
